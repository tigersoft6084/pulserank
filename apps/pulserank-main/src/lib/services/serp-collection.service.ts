...
export class SERPCollectionService {
    ...
    try {
      ...
      for (let i = 0; i < keywordsToUpdate.length; i += batchSize) {
        ...
        const batchPromises = batch.map((keyword) =>
          this.collectSERPDataForKeyword(
            keyword as unknown as KeywordWithHistory
          )
        );
   ...
   */
  private async collectSERPDataForKeyword(keyword: KeywordWithHistory) {
    try {
      ...
      if (!serpResponse.tasks?.[0]?.result?.[0]) {
        throw new ExternalAPIError(
          `No SERP data returned for keyword: ${keyword.keyword}`
        );
      }
