...
export default function DetailKeywordsTab({
  campaignId,
}: {
  campaignId: string;
}) {
  const t = useTranslations("campaigns.detailKeywordsTab");
  ...
  const flatRankings = useMemo(() => {
    return Object.entries(groupedRankings).flatMap(
      ([keyword, keywordRankings]) =>
        keywordRankings.map((ranking) => ({ ...ranking, keyword })),
    );
  }, [groupedRankings]);
  ...
  const sortedGroupedRankings = useMemo(() => {
    ...
    sortedData.forEach((item) => {
      ...
      if (!grouped[typedItem.keyword]) {
        grouped[typedItem.keyword] = [];
      }
      grouped[typedItem.keyword].push(typedItem);
    });
    return grouped;
  }, [sortedData]);
          ...
          <Table>
            ...
            <TableBody>
              {Object.entries(sortedGroupedRankings).map(
                ([keyword, keywordRankings]) => {
                  return keywordRankings.map((ranking, index) => {
                    // Dynamic flag logic
                    const countryCode = baseToCountryCode[ranking.base];
                    const Flag = countryCode ? flagMap[countryCode] : null;

                    return (
                      <TableRow key={`${keyword}-${ranking.site}-${index}`}>
                        {index === 0 && (
                          <TableCell
                            rowSpan={keywordRankings.length}
                            className="align-top"
                          >
                            <div className="flex items-center gap-2">
                              {Flag ? (
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Favicon url={ranking.site} size={16} />
                            {ranking.site}
                          </div>
                        </TableCell>
                        {showDetails && (
