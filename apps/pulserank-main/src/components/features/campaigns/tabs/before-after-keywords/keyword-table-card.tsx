...
export function KeywordTableCard({
  keyword,
  keywordRankings,
  dateFrom,
  dateTo,
  ...
  const { sortedData, sortConfig, handleSort } = useTableSort(
    keywordRankings as unknown as Record<string, unknown>[],
  );

  // Get flag from first ranking (they should all have the same base)
  const firstRanking = keywordRankings[0];
  const countryCode = firstRanking
    ? baseToCountryCode[firstRanking.base]
    : undefined;
        ...
        <Table>
          ...
          <TableBody>
            {sortedData.map((ranking, i) => {
              ...
              return (
                <TableRow key={typedRanking.site + i}>
                  <TableCell>
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <Favicon url={typedRanking.site} size={16} />
                        <Label>{typedRanking.site}</Label>
                      </div>
                      <RankingHistoryModal
                        ranking={typedRanking}
                        site={typedRanking.site}
                      />
                    </div>
                  </TableCell>
