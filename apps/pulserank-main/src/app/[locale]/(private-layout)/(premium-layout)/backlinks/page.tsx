...
import {
  useGetBacklinks,
  type Backlink,
} from "@/hooks/features/backlinks/use-backlinks";
...
export default function BacklinksPage() {
  ...
  } = useGetBacklinks(page, pageSize, Array.from(flagFilters));
  ...
  if (!backlinksData?.data || backlinksData.data.length === 0) {
    return (
  ...
  return (
    <div className="space-y-6">
      <Table>
        ...
        <TableBody>
          {sortedData.map((backlink, index) => {
            const typedBacklink = backlink as unknown as Backlink;
            return (
              <TableRow key={index}>
      ...
      <PaginationControls
        currentPage={page}
        totalPages={backlinksData?.pagination?.pages || 1}
        totalCount={backlinksData?.pagination?.total || 0}
        pageSize={pageSize}
